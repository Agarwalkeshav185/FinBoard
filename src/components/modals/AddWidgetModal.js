...
export default function AddWidgetModal({ isOpen, onClose, onAdd }) {
  ...
  const handleSubmit = (e) => {
    ...
    if (widgetConfig.selectedFields.length === 0) {
      alert('Please select at least one field');
      return;
    }

    // Ensure refresh interval is a valid number
    const refreshInterval = typeof widgetConfig.refreshInterval === 'number' 
      ? widgetConfig.refreshInterval 
      : parseInt(widgetConfig.refreshInterval) || 60;
        ...
        <div>
          ...
          <input
            ...
            onChange={(e) => {
              ...
              // Allow empty string during editing, otherwise parse as number
              setWidgetConfig({ 
                ...widgetConfig, 
                refreshInterval: value === '' ? '' : parseInt(value) || 10 
              });
            }}
            onBlur={(e) => {
              // Ensure valid number when leaving the field
              const value = parseInt(e.target.value);
              if (isNaN(value) || value < 10) {
                setWidgetConfig({ ...widgetConfig, refreshInterval: 60 });
              } else if (value > 3600) {
                setWidgetConfig({ ...widgetConfig, refreshInterval: 3600 });
              }
            }}
            placeholder="60"
            className="w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-lg focus:ring-2 focus:ring-blue-500 dark:bg-gray-700 dark:text-white"
